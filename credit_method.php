<?php
	...
	else if(!isset($_POST["use_credits"]))
	{
		$_SESSION["msg"]="Mention some credits first!";
	...
	date_default_timezone_set("Asia/Kolkata");
		...
		$total_credits=$_SESSION["credits"];
		$charges=$_SESSION["delivery_charges"];
		$total_cost=$total_cost+$charges;
		$new_orders=++$old_orders;
		$arr=$_SESSION["cart"];
		$add=$_SESSION["delivery_address"];
		$entered_credits=$_POST["use_credits"];
		$left=0;
		$new_credits=$old_credits-$entered_credits+$total_credits;
		if($entered_credits>$total_cost)
		{
			$_SESSION["msg"]="Entered Credits aren't correct!";
				header("location:payment.php");
				exit;
		}
		else
		{
			$left=$total_cost-$entered_credits;
		}
		...
		$sql2="insert into orders(order_id,user_id,addressid,date,total_cost,total_credits,status,delivery_charges) values('$order_id','$id','$add','$date','$left','$total_credits','placed','$charges')";
